...
public class FarmingServiceFilterTests
{
    ...
    private FarmingService Create() => new(_uow.Object, _qr.Object);
    ...
    public async Task GetAreas_FiltersBySearchAndActive()
    {
        ...
        var result = await Create().GetAreasAsync(new FarmingAreaFilter("Khu", true, 1, 50));
    ...
    public async Task GetBoxes_FiltersByStatusAndCode()
    {
        ...
        var result = await Create().GetBoxesAsync(new BoxFilter(null, rowId, "A", "active", true, 1, 50));
        result.Data!.TotalCount.Should().Be(1);
